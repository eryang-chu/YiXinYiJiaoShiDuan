...
- (void)initialize {
    ...
    districtWidth = self.frame.size.width - 30;
    [self loadData];
...
- (void)initializePageControl {
    ...
    _pageControl.currentPageIndicatorTintColor = [UIColor redColor];//HEX_COLOR(0xBEBEBE);
...
- (void)loadData {
    ...
    if (_datasourceImages.count == 0) {
        ...
        if ([self.datasource respondsToSelector:@selector(placeHolderImageOfZeroBannerView)]) {
            CGRect imgFrame = CGRectMake(5, 5, districtWidth, CGRectGetHeight(_scrollView.frame)-10);
            ...
            [imageView layerCornerRadius:@(5) borderWidth:@(1) borderColor:[UIColor clearColor]];
    ...
    CGFloat contentWidth = districtWidth;//CGRectGetWidth(_scrollView.frame)-20;
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        CGRect imgFrame = CGRectMake(imgRect.origin.x+15+5/2, imgRect.origin.y+5, districtWidth-5, CGRectGetHeight(imgRect)-10);
        ...
        [imageView layerCornerRadius:@(10) borderWidth:@(1) borderColor:[UIColor clearColor]];
        [_scrollView addSubview:imageView];
    }
    
    if (self.isContinuous && _datasourceImages.count > 1) {
//        _scrollView.contentOffset = CGPointMake(CGRectGetWidth(_scrollView.frame), 0);
        _scrollView.contentOffset = CGPointMake(districtWidth, 0);
    }
...
//                [self moveToTargetPosition:CGRectGetWidth(_scrollView.frame) * (page + 2) withAnimated:animated];
                [self moveToTargetPosition:districtWidth * (page + 2) withAnimated:animated];
            }else {
//                [self moveToTargetPosition:CGRectGetWidth(_scrollView.frame) * (page + 1) withAnimated:animated];
                [self moveToTargetPosition:districtWidth * (page + 1) withAnimated:animated];
            }
        }else {
...
//        [self moveToTargetPosition:CGRectGetWidth(_scrollView.frame) * page withAnimated:animated];
        [self moveToTargetPosition:districtWidth * page withAnimated:animated];
    }
...
{
    CGFloat item_width = districtWidth;//CGRectGetWidth(scrollView.frame);
    
    NSInteger page = (scrollView.contentOffset.x + item_width * 0.5) / item_width-1;
...
//    NSInteger page = (NSInteger)(_scrollView.contentOffset.x / CGRectGetWidth(_scrollView.frame));
    NSInteger page = (NSInteger)(_scrollView.contentOffset.x / districtWidth);
