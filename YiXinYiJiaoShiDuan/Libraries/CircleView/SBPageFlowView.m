...
- (void)setIsHiddenPageControl:(BOOL)isHiddenPageControl {
    _isHiddenPageControl = isHiddenPageControl;
    self.pageControl.hidden = isHiddenPageControl;
}

- (UIPageControl *)pageControl {
    if (!_pageControl) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.frame) - 15, self.bounds.size.width, 10.)];
        _pageControl.userInteractionEnabled = NO;
        _pageControl.backgroundColor = [UIColor clearColor];
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:191./255 green:0 blue:0 alpha:1];
...
- (void)layoutSubviews{
    ...
    if (_needsReload) {
        ...
        self.pageControl.numberOfPages = [self.dataSource numberOfPagesInFlowView:self];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)aScrollView {
    ...
    if(_orientation == FlowViewOrientationHorizontal) {
        // 往下翻一张
        if(x >= (2*_pageSize.width - _paddingWidth)) {
