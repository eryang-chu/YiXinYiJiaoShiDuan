...
- (void)setupSubviews {
    ...
    UIButton* btn = [self.storageAlltitleBtn_mArr lastObject];
    CGFloat f = SCREEN_WIDTH - btn.frame.origin.x - btn.frame.size.width;
    if( f>0 ){
        CGFloat subWidth = 0;
        for(UIButton* b in self.storageAlltitleBtn_mArr ){
            subWidth += b.frame.size.width;
        }
        CGFloat xMargin = 0;
        for(UIButton* b in self.storageAlltitleBtn_mArr ){
            b.frame = CGRectMake(xMargin, b.frame.origin.y, (b.frame.size.width/subWidth)*SCREEN_WIDTH, b.frame.size.height);
            xMargin+=b.frame.size.width;
        }
    }
